...
E=[
(r"Estatística do zero",r"Escore z (padronização)",r"z=\frac{x-\bar x}{s}\quad\text{ou}\quad Z=\frac{X-\mu}{\sigma}",r"Quantos desvios-padrão um valor está da média.",r"Sem unidade.",r"Módulo Estatística do zero; Aula 2",r"A estatística t é um escore z da estimativa.",r"padronizacao z score normal padrao"),
(r"Estatística do zero",r"Coeficiente de variação",r"CV=\frac{s}{\bar x}",r"Dispersão relativa à média.",r"Sem unidade (muitas vezes em %).",r"Módulo Estatística do zero",r"Só faz sentido para variáveis positivas.",r"cv dispersao relativa"),
(r"Estatística do zero",r"Probabilidade condicional e independência",r"P(A\mid B)=\frac{P(A\cap B)}{P(B)},\qquad\text{independentes: }P(A\cap B)=P(A)P(B)",r"Probabilidade de A sabendo que B ocorreu.",r"Probabilidade (0 a 1).",r"Módulo Estatística do zero; Aula 2",r"",r"condicional independencia"),
(r"Estatística do zero",r"Binomial",r"P(X=k)=\binom nk\pi^k(1-\pi)^{n-k},\quad E=n\pi,\quad Var=n\pi(1-\pi)",r"Número de sucessos em n tentativas independentes.",r"Contagem (sem unidade).",r"Módulo Estatística do zero; Lista 1 Q27",r"",r"binomial bernoulli contagem"),
...
(r"Probabilidade (Aula 2)",r"Viés e erro quadrático médio",r"Vi\acute es=E[\hat\theta]-\theta,\qquad EQM=Var(\hat\theta)+Vi\acute es^2",r"Viés: erro do centro. EQM: erro total médio.",r"Viés na unidade do parâmetro; EQM ao quadrado.",r"Lista 1 Q8; Aula 5; Lista em sala Ex. 1 e 2",r"EQM = E[(estimativa − θ)²]. Mais repetições não corrigem o viés.",r"eqm erro quadratico medio vies lista em sala"),
...
(r"Propriedades do MQO (Aula 5)",r"Ausência de viés",r"E(\hat\beta_1)=\beta_1,\quad E(\hat\beta_0)=\beta_0\quad\text{(RLS.1–RLS.4)}",r"O centro da distribuição amostral é o parâmetro.",r"—",r"Aula 5 ★; Lista 3 Q1",r"Não diz que cada estimativa está perto; RLS.5 não é usada.",r"nao viesado vies"),
(r"Propriedades do MQO (Aula 5)",r"Variância da inclinação",r"Var(\hat\beta_1\mid X)=\frac{\sigma^2}{SQT_x}\quad\text{(RLS.1–RLS.5)}",r"Precisão: cai com mais variação em x, sobe com mais ruído.",r"(Unid. y / unid. x) ao quadrado.",r"Aula 5 ★; Lista 3 Q4 e Q10",r"Precisa de RLS.2 e RLS.5.",r"variancia beta1 precisao"),
...
(r"Propriedades do MQO (Aula 5)",r"Erros-padrão",r"ep(\hat\beta_1)=\frac{\hat\sigma}{\sqrt{SQT_x}},\qquad ep(\hat\beta_0)=\frac{\hat\sigma\sqrt{\sum x_i^2}}{\sqrt{n\,SQT_x}}",r"Estimativas do desvio-padrão dos estimadores.",r"A mesma do coeficiente correspondente.",r"Aulas 5 e 6; Lista 3",r"ep = raiz da variância estimada.",r"erro padrao ep"),
...
(r"Inferência (Aula 6)",r"Estatística t",r"t=\frac{\hat\beta_1-\beta_{1,0}}{ep(\hat\beta_1)}\sim t_{n-2}",r"Distância até \(H_0\) em erros-padrão.",r"Sem unidade.",r"Aula 6 ★; Lista 3",r"Nunca “aceitar” \(H_0\).",r"t teste hipotese"),
(r"Inferência (Aula 6)",r"Intervalo de confiança",r"IC=\hat\beta_1\pm t_{\alpha/2,\,n-2}\cdot ep(\hat\beta_1)",r"Valores de \(\beta_1\) compatíveis com os dados.",r"A mesma de \(\beta_1\).",r"Aula 6; Lista 3 Q7 e Q10",r"0 fora do IC ⇔ rejeita \(H_0:\beta_1=0\).",r"intervalo confianca ic"),
...
(r"Prévia da P2 (fora da P1)",r"Variância do erro na regressão múltipla",r"\hat\sigma^2=\frac{SQR}{n-k-1}",r"Com k explicativas mais o intercepto, perdem-se k + 1 graus de liberdade.",r"Unidade de y ao quadrado.",r"Módulo Prévia da P2",r"Não use n − 2 com mais de uma explicativa.",r"multipla graus liberdade"),
(r"Prévia da P2 (fora da P1)",r"R² ajustado",r"\bar R^2=1-\frac{SQR/(n-k-1)}{SQT/(n-1)}",r"R² que penaliza variáveis a mais.",r"Sem unidade.",r"Módulo Prévia da P2",r"O R² comum nunca cai ao incluir variáveis.",r"r2 ajustado"),
(r"Prévia da P2 (fora da P1)",r"Variância na múltipla e VIF",r"Var(\hat\beta_j)=\frac{\sigma^2}{SQT_j(1-R_j^2)},\qquad VIF_j=\frac{1}{1-R_j^2}",r"Multicolinearidade infla a variância.",r"(Unid. y / unid. \(x_j\))².",r"Módulo Prévia da P2",r"Não causa viés; VIF &gt; 10 é só um alerta.",r"multicolinearidade vif"),
(r"Prévia da P2 (fora da P1)",r"Teste F de significância conjunta",r"F=\frac{R^2/k}{(1-R^2)/(n-k-1)}\sim F_{k,\,n-k-1}",r"Testa se todas as inclinações são zero ao mesmo tempo.",r"Sem unidade.",r"Aula 6 (teaser); módulo Prévia da P2",r"Pode rejeitar mesmo com todos os t pequenos (multicolinearidade).",r"teste f conjunto"),
(r"Prévia da P2 (fora da P1)",r"Autocorrelação AR(1) e Durbin–Watson",r"u_t=\rho u_{t-1}+e_t,\qquad d=\frac{\sum_{t\ge2}(\hat u_t-\hat u_{t-1})^2}{\sum\hat u_t^2}\approx2(1-\hat\rho)",r"Erros correlacionados no tempo; d ≈ 2 indica ausência.",r"Sem unidade.",r"Módulo Prévia da P2",r"Não causa viés sob exogeneidade estrita, mas o ep usual fica errado (use HAC).",r"autocorrelacao serial durbin watson"),
